...
class RealModernFrontierIntegrationTests(unittest.TestCase):
    def test_rejected_n256_w28_reference_point(self):
        assessment = assess_modern_candidate(256, 128, 28, max_repetitions=512)
    ...
    def test_measured_128_screen_candidate(self):
        assessment = assess_modern_candidate(1064, 532, 117, max_repetitions=512)
        self.assertEqual(assessment.upstream_package_version, "2.1.1")
        self.assertEqual(assessment.upstream_algorithm, "MayOzerov")
        self.assertAlmostEqual(assessment.upstream_time_bits, 128.611921, places=5)
        self.assertEqual(assessment.effective_attack, "MayOzerov")
        self.assertAlmostEqual(assessment.effective_attack_bits, 128.611921, places=5)
        self.assertGreater(assessment.support_enumeration_bits, 500.0)
        self.assertEqual(assessment.repetitions, 220)
        self.assertEqual(assessment.cutoff_ones, 61)
