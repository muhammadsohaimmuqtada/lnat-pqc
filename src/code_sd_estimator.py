...
The output is an attack estimate, not a security proof.  In particular, a
reported time value is meaningful only under the upstream estimator's cost
model and options.
"""
...
def estimate_upstream_isd(
    ...
    *,
    ...
    ``bit_complexities=1`` asks the upstream package for bit-complexity
    estimates rather than field-operation counts.  The package version is
    checked by default so CI results do not silently drift when upstream cost
    models or optimizers change.
    """
