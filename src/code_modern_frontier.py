...
The serious parameter screen must use the cheapest implemented public attack,
not the LNAT seed length and not a local reference model in isolation.

For a candidate ``(n, k, w)`` this module computes:
...
and combines that attack screen with the repository's exact fixed-weight
intersection model and conservative full-KEM failure bound.

Estimator output is an attack-screening model, not a proof of cryptographic
security. Crossing a numeric floor here only means that the candidate survives
the attack models currently wired into this repository.
"""
