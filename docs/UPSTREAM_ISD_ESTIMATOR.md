...
The upstream binary Syndrome Decoding estimator includes finite-parameter models for algorithms including Prange, Stern, Dumer, BJMM variants, Both-May, May-Ozerov, and ball-collision methods.
...
python experiments/upstream_isd_probe.py --n 256 --k 128 --weight 48 --top 8
```
...
3. parameter candidates are rejected when stronger modeled attacks are clearly too cheap;
4. no estimator output by itself proves a security level.

The active Stern-aware frontier currently uses `(n=256,k=128,w=48)` as its deterministic research regression. This cross-check is specifically intended to test whether that point survives stronger generic decoding models.
